...
Modified to plot solar panel of different conversion rates 05/02/2018
"""
import numpy as np
...
def main():
    exp_name = ['ExpSolar0', 'ExpSolar0_rate30', 'ExpSolar0_rate45']
    variable_name =['temp0', 'prec']
    fig_titles = ['Solar farm (15%)', 'Solar farm (30%)', 'Solar farm (45%)']
	...
	if i < 3:
	    ...
	    if i==1:
                cube = load_diff_data_nomask(exp_name[i], variable_name[0])
	    else:
	    ...
	    if i==4:
                cube = load_diff_data_nomask(exp_name[i%3], variable_name[1])
	    else:
...
#            cube = load_diff_data(exp_name[i%3], variable_name[1])
   ...
   # Add axes to the figure, to place the colour bar 
    ...
    plt.savefig('figure_solar_rates.pdf')
    iplt.show()
